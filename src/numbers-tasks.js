...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const multy = x1 * x2 + y1 * y2;
  const modul1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modul2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cos = multy / (modul1 * modul2);
...
function toNumber(value, def) {
  if (Number.isInteger(+value) || value instanceof Number) return +value;
  return def;
}
...
function getCountOfOddNumbers(number) {
  if (number % 2 === 0) return Math.abs(number / 2);
